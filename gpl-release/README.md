...
The build system auto-detects your platform (Linux, macOS, FreeBSD, OpenBSD,
NetBSD, DragonFly BSD) via `uname -s` and selects the appropriate libraries
and flags. If you are on an unusual Unix variant and linking fails, you can
override `LIBS` in the generated `Makefile`.
...
- **header.h**: Game configuration constants and gameplay settings
- **Makefile**: Admin login, install paths, and build type
...
3. **Edit Include/header.h** and configure as needed:
   - `OWNER`: Administrator name and contact information
   - Game balance and timing parameters

4. **Edit Makefile** and configure as needed:
   - `LOGIN`: Administrator login ID (defaults to current user via `whoami`)
   - `PREFIX`: Install root. FHS-compliant defaults are used: `/usr/local`
     when building as root, `$HOME/conquerv5` otherwise.
   - `BUILD`: `debug` (default) or `release` for an optimized build
   - `TOPDIR`, `DATADIR`, `BINDIR`: Derived from `PREFIX` (data files go to
     `$PREFIX/share/conquerv5`, executables to `$PREFIX/bin`). Override only
     if you want custom locations.
   - `SYSFLG` and `LIBS` are auto-detected via `uname -s` for Linux, macOS,
     FreeBSD, OpenBSD, NetBSD, and DragonFly BSD. Override these only on
     unusual Unix variants where detection fails.

   Editing the Makefile once is the recommended approach — values set there
   apply to every subsequent `make` invocation in the build sequence
   (`make Makefiles`, `make build`, `make install`).
